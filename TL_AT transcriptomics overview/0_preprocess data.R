...
subject_w_POPs <- data_wide %>%
  dplyr::select(key, any_of(chemical_variables)) %>%
  filter(rowSums(is.na(.)) < length(chemical_variables)) %>%
  dplyr::select(key) %>% unlist
...
(total_ddt_tertile <- quantile(data %>% dplyr::filter(visit_new == 0) %>% ungroup %>% dplyr::select(total_ddt_adipose_0) %>% unlist, c(0:3/3)))
...
# Create Working dataset for baseline only
data_baseline <- data %>% 
  dplyr::filter(visit == 0) %>% 
  dplyr::select(key, all_of(exposure), all_of(outcome), all_of(covariates))

# Merge cohort data to RNAseq data
RNAseq$metadata <- left_join(
  RNAseq$metadata %>% mutate(key = as.factor(key)),
  data_baseline,
  by = "key")
